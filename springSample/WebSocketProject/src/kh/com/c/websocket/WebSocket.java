...
public class WebSocket extends TextWebSocketHandler{
	...
	//Service접근 (db 처리를 하기위해 )
	    ...
	    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
	    	...
	    	ChatMemberDto userRoom = projectChatService.getRoomMember(new ChatMemberDto(0, userId, "",""));
	    	...
	    	projectChatService.updateRoomMember(new ChatMemberDto(0, userId, "", priroom));
	    	...
	    	logger.info("세션추가:"+session.getId()+"접속자아이디:"+mem.getId()+" 현재채팅접속자:"+sessionList.size()+"명");
	  ...
	  //통신 연결끊었을때실행
	    ...
	    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
	    	...
	    	//2. 접속을끊을 때 해당 아이디로 DB에서 어느 방에 존재하는지 확인한다.
	     	ChatMemberDto member = projectChatService.getRoomMember(new ChatMemberDto(0, userId, "", ""));
